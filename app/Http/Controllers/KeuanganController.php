...
class KeuanganController extends Controller
{
    public function index(request $request){
        ...
        $lengkap=$tahun.'-'.$bulan.'-'.$tgl;
        return view('keuangan.index',compact('menu','bulan','tahun','tgl','lengkap'));
    }
    ...
    public function get_data(request $request){
        $query=VKeuangan::query();
        if($request->kategori!=""){
            $data=$query->where('status',$request->kategori);
        }
        $data=$query->where('tahun',$request->tahun)->where('bulan',$request->bulan)->orderBy('created_at','Desc')->get();
                ...
                ->addColumn('action', function($data){
                        ...
                        <div class="btn-group btn-group-sm dropup m-r-5">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                ...
                                <a href="javascript:;" onclick="view_data(`'.$data->no_transaksi.'`,'.$data->kategori.')" class="dropdown-item"><i class="fas fa-search fa-fw"></i> View</a>
                            </div>
                        </div>
                         ';
